...
public class FR_creation_Multi_Room extends BaseTestDec25{
	
	@Test()
	  public void NewCare_DashBoard_Test() throws Exception{
		test = rep.startTest("FR_creation_Multi_Room");
		test.log(LogStatus.INFO, "Starting the test -- creation_Multi_Room FR");
		initialization();
	  	openBrowser("chrome");
	  	navigate("CareURL");
	  	typeTextUser("userLoginID_xpath", "careadmin1");
	  	typeTextPass("userLoginPass_id", "bbcl@gr1");
	  	FR_WO_Multi_Room_ScreenShot();
	  	click("UserSubmit_xpath");
	  	System.out.println(dr.getTitle());
	  	Thread.sleep(2000);
	  	switchFrameCareDashBoard("switchFrameCareDashBoard_xpath");	
	  	// Witch can manage and build project reporting and documentation for center piece of information.
	  	dr.switchTo().frame(0);
	  	NewDashBoard_CustomerID_Search("Cust_Search_id", "2022199");
	...
	if(WorkOrder_RTN_Update_Window_Open.isDisplayed()){
		...
		Thread.sleep(6000);
		Close_Popup_Window_Click("Close_Popup_Window_Click_xpath");
		System.out.println("Multi Room FR created Successfully");
		Thread.sleep(15000);
		FR_WO_Multi_Room_ScreenShot();
		dr.close();
	}
